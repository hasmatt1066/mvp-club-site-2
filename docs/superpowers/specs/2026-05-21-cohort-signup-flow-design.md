...
| 2026-05-25 | **Plain JS replaces React Email + TS.** vercel dev (which delegates to astro dev) couldn't natively import `.jsx` or `.ts` files in the api/* runtime, so React Email + cohort.ts broke locally. Switched to template-literal HTML strings in `.js` files (`src/emails/cohortPaidEmail.js`, `cohortWaitlistEmail.js`) and renamed `cohort.ts` → `cohort.js`. Visual output is unchanged. Also added a dev-mode signature bypass in `api/stripe-webhook.js` for the same vercel-dev-doesn't-honor-bodyParser-false issue. Production verifies signatures normally. | Matt |
| 2026-05-25 (later) | **Cohort signup flow shipped to production.** Live Stripe product + Payment Link + webhook endpoint created. Live env vars set in Vercel. Pushed to `main`. `/ai-summer-camp` is publicly live with real Stripe payments. Email copy updated to add Claude desktop + Pro subscription as a pre-cohort prep step. | Matt |
...
- ~~Email templates as inline strings in webhook handlers. We're using React Email components for type-safety and easier preview/iteration.~~ **Superseded 2026-05-25:** React Email + JSX templates didn't work under `vercel dev` (astro dev runtime can't import `.jsx`). We switched to plain JS template-literal HTML in `src/emails/cohortPaidEmail.js` and `cohortWaitlistEmail.js`. Visual output is identical. The React Email packages are still in `package.json` but unused; they can be pruned in a separate cleanup commit.
